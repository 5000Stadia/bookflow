...
def admitted(monkeypatch):
    ...
    ``journal delete`` is registered, but the activated permission catalog carries no
    descriptor for it until its delta is sequenced with the catalog re-layering in flight, so
    dispatch's activation gate refuses the command before anything else on the path runs.
    Only that descriptor lookup is stood in for: the explicit per-user grant, the
    ``ledger.read`` requirement and every other admission still run exactly as they ship,
    inside ``journal_deletions.admit``.
    ``test_the_catalog_delta_is_the_only_gate_left_on_this_family`` pins what is stood in for.
    """
    from bookflow.hub import access
    original = access.require_command_activation

    def gate(s, cmd):
        if cmd.name == 'journal delete':
            return None
        return original(s, cmd)

    monkeypatch.setattr(access, 'require_command_activation', gate)
...
def test_co55_declaration_is_exact_and_the_storage_is_immutable(ledger, admitted):
    ...
    migration = importlib.import_module('bookflow.storage.company_migrations.versions.0055_journal_deletions')
    assert migration.revision == 'co0055' and migration.down_revision == 'co0052'
...
def test_the_catalog_delta_is_the_only_gate_left_on_this_family(ledger):
    """Everything above runs with one descriptor stood in for. This is that descriptor.

    ``journal delete`` is registered and complete; the activated permission catalog has no
    entry for it until this family's delta lands, and dispatch refuses an unknown command on
    an activated root. Delete this test with the delta.
    """
    post = entry(ledger, memo='Waiting on the catalog', amount='1.10', date='2017-09-10')
    with pytest.raises(BookflowError) as refused:
        ledger['run']('journal delete', dict(journal=post['id'],
            expected_version=post['version'], operation_key='no-descriptor'),
            reason='Remove duplicate entry')
    assert refused.value.code == 'E_PERMISSION'
    assert refused.value.details['reason'] == 'command_unavailable'
