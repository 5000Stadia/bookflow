...
def test_inline_and_replacement_current_denies_preserve_money_and_storage(captures):
    c = captures
        ...
        def project(current):
            ...
            for raw, seq in c['rows']:
                ...
                with binding.hosted_reader(host, current, request_id=ctx.request_id) as reader:
                    ...
                    outputs.append(projection._disclose_company(audience, c['cid'], value, cutoff=seq).model_dump(mode='json', by_alias=True))
            event, seq = c['manifest_event']
            ctx = Context.new('http', 'inline-edit-origins')
            with binding.hosted_reader(host, current, request_id=ctx.request_id) as reader:
                open_selected(reader, projection.HistorySelection(mode='show', company=c['cid'], event=event), ctx)
                audience = projection.make_audience(reader)
                audience.require(c['cid'], (('ledger.read', 'member'),))
                manifest = projection._disclose_company(audience, c['cid'],
                    views.DepositAuditManifest.model_validate(c['manifest']), cutoff=seq).model_dump(mode='json', by_alias=True)
            return outputs, manifest

        allowed, allowed_manifest = project(cred)
        ...
        def deny():
            with host._commit_hooks.operation('dispatch.apply', host._hub):
                with binding.hosted_operation(host, cred, request_id=CONTEXT.request_id, purpose='apply') as operation:
                    operation.apply(admin.PutMembership(uid, ScopeKey('company', c['cid']), admin.Version(1),
                        'owner', denies=('account', 'payment-method', 'custom-field')), audit=CONTEXT)
                    host._commit_hooks.commit(host._hub, 'dispatch.apply')
        host.submit(deny)
        denied, denied_manifest = project(OSBinding.capture(host, os_login()))
        for index, (yes, no) in enumerate(zip(allowed, denied, strict=True)):
