...
def test_deposit_catalog_and_publication_inventory_keep_both_owners():
    ...
    assert commands.keys() == actions.keys() == {'deposit '+x for x in ('query','show','items','history','post','sources','update','void')}
    for name in commands:
        ...
        assert policy == ('reader_bound_public_detail_proof'
                          if name in ('deposit query','deposit show','deposit items','deposit history')
