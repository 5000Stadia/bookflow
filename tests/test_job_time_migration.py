"""co0053 admits a fourth work kind into three CHECKs and the two guards that mirror them.
...
M = importlib.import_module('bookflow.storage.company_migrations.versions.0053_job_time')
...
def test_the_migration_sits_in_the_chain_where_it_says_it_does():
    assert M.revision == 'co0053' and M.down_revision == 'co0052'
    assert M.revision in _chain()
    assert HEADS['company'] == 'co0053'
...
def test_a_populated_co0052_database_keeps_every_value_and_every_local_object(tmp_path):
    ...
    with sqlite3.connect(path) as raw:
        ...
        rebuilt = set(M.CHANGED) | set(M.TRIGGERS) | _rebuilt_since(M.revision)
...
def test_a_rewritten_guard_stops_the_migration_rather_than_being_guessed_at(tmp_path, name):
    ...
    with open_database(path, writable=True) as db:
        ...
        assert 'co0053' in str(caught.value) or 'co0053' in str(caught.value.__cause__)
