...
# identities. Each is dispositioned below: the twelve with a designated transport witness are
# mapped to it through that test's own COMMANDS set, and `bill delete` and `payment delete`
# deliberately are not -- their executed evidence is HTTP and real Chrome, with no CLI/MCP
# matrix witness anywhere, so they fall through to a pending row rather than being counted as
# parity they do not have.
...
def execution_map():
    ...
    for cmd in commands:
        witness = ('tests/test_mcp_registry_browsing.py::test_list_browsing_four_interfaces' if cmd.name in BROWSING_COMMANDS else
                   ...
                   'tests/test_purchase_deletion_transports.py::test_cli_and_source_bound_mcp_delete_preview_refusal_replay_and_history' if cmd.name in PURCHASE_DELETE_COMMANDS else
                   'tests/test_sales_deletion_transports.py::test_cli_and_source_bound_mcp_delete_preview_refusal_replay_and_history' if cmd.name in SALES_DELETE_COMMANDS else
                   'tests/test_receiving_surfaces.py::test_receiving_and_linked_bill_cross_all_four_actual_transports' if cmd.name in RECEIVING_COMMANDS else
                   'tests/test_permission_setup_surfaces.py::test_permission_setup_crosses_all_four_actual_transports' if cmd.name in SETUP_SURFACE_COMMANDS else
