...
def test_committed_payment_denial_keeps_one_effect_and_reference_cannot_reexecute(hosted, payment_graph, monkeypatch):
    ...
    events = hosted.ok('audit.list', {'command': 'payment update', 'limit': 200}, company=hosted.company_id)
    assert len(events['items']) == 1 and events['items'][0]['reason'] == 'Postcommit boundary'
