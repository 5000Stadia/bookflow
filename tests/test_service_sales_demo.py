...
def test_reference_expectations_include_exact_commercial_arithmetic():
    ...
    assert (annual["trial_balance"], annual["income"], annual["net_assets"]) == (8048600, 6457000, 7457000)
...
def test_seed_known_balances_counts_and_readonly_preview(reference_client, company, prefix):
    client, _ = reference_client
    checking, trial, journals = (624895, 708195, 10) if prefix == "DEMO" else (7267800, 8048600, 36)
    ...
    expected_balances = ({"Checking": 624895, "Accounts Receivable": 13800,
    ...
    assert profit["totals"]["net_income"]["minor_units"] == (151095 if prefix == "DEMO" else 6457000)
    assert sheet["totals"]["total_equity"]["minor_units"] == (651095 if prefix == "DEMO" else 7457000)
    ...
    before = counts()
    # Row19 adds one exempt invoice/revision, two batches and four legs.
    # Row22 adds seven documents/eight revisions, eleven batches and 28 legs;
    # only four of its revisions are sales profiles (the other four are cash).
    assert before["transactions"] == journals + 18 + 1 + 7
    assert before["sales_profiles"] == 24 + 1 + 4
    assert before["sales_line_profiles"] == 42 + 1 + 4
    assert before["sales_tax_components"] == 38
    assert (before["transaction_revisions"], before["posting_batches"], before["posting_lines"]) == (
        (47, 78, 293) if prefix == "DEMO" else (70, 98, 326))
