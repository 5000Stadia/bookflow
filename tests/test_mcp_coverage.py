...
def test_registry_execution_ledger_has_no_unclassified_commands(tmp_path):
    rows = execution_map()
    assert len(rows) == 330
    assert sum(row['coverage'] == 'four_surface_scenario' for row in rows) == 325
...
def test_material_variant_inventory_is_finite_and_does_not_hide_open_cases():
    ...
    assert len(mapped)==len(variant_policies())==21
    assert sum(len(group['paths']) for group in mapped)==2072
