...
class LeasedDownload(StreamingResponse):
    ...
    async def chunks(self):
        ...
        while True:
            await run_in_threadpool(transfer.check_output)
            chunk = await run_in_threadpool(transfer.reader.read, CHUNK_BYTES)
            if not chunk:
                break
            size += len(chunk)
...
def install(app, host, *, credential, lookup, selector_of, make_context, secret_of):
    ...
    async def transfer_command(company_id: str, route: str, request: Request):
        def begin():
            ...
            return guard(lambda: HostedTransfer(host, cmd, raw, make_context(request, cred),
                         cred.user_id, cred.login, selector, source, dry_run, recheck), cred.hub_admin)

        transfer = await run_in_threadpool(begin)
        keep = False
        try:
            ...
            while True:
                ...
                if transfer.cmd.transfer.direction == "output":
                    ...
                    for start in range(0, len(chunk), CHUNK_BYTES):
                        await run_in_threadpool(transfer.body.write, chunk[start:start + CHUNK_BYTES])
            if transfer.cmd.transfer.direction == "input":
                return await run_in_threadpool(lambda: guard(transfer.finish_input))
            response = LeasedDownload(transfer)
            keep = True
            return response
        finally:
            if not keep:
                with anyio.CancelScope(shield=True):
                    await run_in_threadpool(transfer.close)
