...
@command("report profit-and-loss", scope="company", required_role="member", capability="reports",
    description="Uses the company cash/accrual report preference unless basis is supplied. Income, costs and net profit for inclusive accounting dates. Own-account rows are paged; statement totals cover all accounts. Ordinary journal transfers affect income exactly as entered.",
    input_model=ProfitAndLossInput, output_model=ProfitAndLossOutput,
    error_codes=["E_CASH_BASIS_EVIDENCE", "E_QUERY_STALE", "E_VALUE_RANGE"])
...
@command("report balance-sheet", scope="company", required_role="member", capability="reports",
    description="Uses the company cash/accrual report preference unless basis is supplied. Assets, liabilities and equity as of date_to, with posted equity, derived prior earnings and current fiscal-year income shown separately. Own-account rows are paged; totals cover the whole statement.",
...
@command("report sales-by-customer", scope="company", required_role="member", capability="reports",
    ...
    error_codes=["E_CASH_BASIS_EVIDENCE", "E_QUERY_STALE", "E_VALUE_RANGE", "E_RECORD_NOT_FOUND"])
