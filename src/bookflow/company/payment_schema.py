...
def define_tables(metadata, column, table, common):
    ...
    applications = T('applications',
        ...
        ident('source_component_key_id', 'Exact-party permanent receipt source; null when a credit supplied the capacity.', True),
        ...
        # One receivable settlement edge, two kinds of source. The column is nullable because
        # NULL is the true value for every receipt row ever written, and the kind of source is
        # derived from which column is present rather than stored as a separate classification.
        ident('credit_source_key_id', 'Exact-party permanent credit source; null when a receipt supplied the capacity.', True),
        ...
        owner(['paying_transaction_id', 'credit_source_key_id'], ['credit_source_keys.transaction_id', 'credit_source_keys.id'], 'application_credit_source'),
    ...
    application_allocations = T('application_allocations',
        ...
        ident('source_component_id', 'Captured revision-local receipt capacity; null when a credit supplied it.', True),
        ident('source_posting_source_id', 'Exact source AR attribution.'),
        ...
        # The credit twin of source_component_id; exactly one of the two is present, which is
        # what the applications_one_source fence asserts on the edge above.
        ident('credit_source_component_id', 'Captured revision-local credit capacity; null when a receipt supplied it.', True),
        ...
        owner(['source_transaction_id', 'source_revision_id', 'credit_source_component_id'], ['credit_components.transaction_id', 'credit_components.revision_id', 'credit_components.id'], 'allocation_credit_source'),
