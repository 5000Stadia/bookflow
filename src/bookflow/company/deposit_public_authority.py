...
current authoritative list-resource vocabulary.
"""
...
# Closed navigation kind -> (record kind, catalog capability). The private
# reader emits exactly these navigation tables; conform() pins the mapping to the
# existing list-resource vocabulary so a new list noun cannot arrive untyped.
...
def conform():
    """Pin the public reference groups to the current authoritative resource catalog."""
    from bookflow.company.lists import LIST_DEFINITIONS
    nouns = {definition.table: definition.noun for definition in LIST_DEFINITIONS.values()}
    # Accounts have a dedicated owner, rather than a generated list definition.
    nouns['accounts'] = 'account'
    for table, (_, capability) in REFERENCE_CAPABILITIES.items():
        if nouns.get(table) != capability:
            raise BookflowError('E_INTERNAL', message='deposit reference vocabulary changed')
    requirements = {c.Requirement(item.name, threshold)
                    for item in c.FROZEN_CATALOG.capabilities for threshold in item.company_thresholds}
    expected = [READ_REQUIREMENT, *((capability, 'member') for _, capability in REFERENCE_CAPABILITIES.values()),
                (CUSTOM_FIELD_CAPABILITY, 'member'), (COMPANY_CAPABILITY, 'member'),
                *((capability, 'member') for capability in ANNOTATION_CAPABILITIES.values())]
    if any(c.Requirement(*requirement) not in requirements for requirement in expected):
        raise BookflowError('E_INTERNAL', message='deposit resource requirement changed')
