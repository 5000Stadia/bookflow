...
def define_tables(metadata, column, table, common):
    ...
    transactions = T('transactions', *common(),
        text('type', 'Business document type: journal_entry, invoice, sales_receipt, payment, deposit, bill or bill_payment.', size=32),
        ...
        sa.CheckConstraint("type IN ('journal_entry', 'invoice', 'sales_receipt', 'payment', 'deposit', 'bill', 'bill_payment')", name='ck_transaction_type'),
    ...
    document_lines = T('document_lines', *created(),
        ...
        text('kind', 'Entered line kind: journal, sale, payment, deposit, purchase or bill_payment.', size=16),
        ...
        *dimensions(), *money_facts(),
        ...
        sa.CheckConstraint(
            ...
            "(kind IN ('sale', 'payment', 'deposit', 'purchase', 'bill_payment') AND account_id IS NULL AND side IS NULL AND amount_minor_units IS NULL AND account_snapshot IS NULL "
