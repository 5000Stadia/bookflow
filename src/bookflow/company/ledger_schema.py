...
def define_tables(metadata, column, table, common):
    ...
    transactions = T('transactions', *common(),
        text('type', 'Business document type: journal_entry, invoice, sales_receipt, payment, deposit, bill, bill_payment or credit_memo.', size=32),
        ...
        sa.CheckConstraint("type IN ('journal_entry', 'invoice', 'sales_receipt', 'payment', 'deposit', 'bill', 'bill_payment', 'credit_memo')", name='ck_transaction_type'),
        ...
        # uq_transaction_type_number is per type, and an invoice and a credit memo share
        # one number series, so per-type uniqueness would let two documents a customer
        # reads as one series carry the same number. This is where that is refused.
        sa.Index('uq_transaction_receivable_number', 'number', unique=True,
                 sqlite_where=sa.text("type IN ('invoice', 'credit_memo')")),
    ...
    document_lines = T('document_lines', *created(),
        ...
        text('kind', 'Entered line kind: journal, sale, payment, deposit, purchase, bill_payment or credit.', size=16),
        ...
        *dimensions(), *money_facts(),
        ...
        sa.CheckConstraint(
            ...
            "(kind IN ('sale', 'payment', 'deposit', 'purchase', 'bill_payment', 'credit') AND account_id IS NULL AND side IS NULL AND amount_minor_units IS NULL AND account_snapshot IS NULL "
