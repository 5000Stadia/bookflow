...
def define_tables(metadata, column, table, common):
    ...
    transactions = T('transactions', *common(),
        text('type', 'Business document type; journal_entry for entered journals.', size=32),
        ...
        sa.CheckConstraint("type = 'journal_entry'", name='ck_transaction_type'),
    ...
    document_lines = T('document_lines', *created(),
        ...
        text('kind', 'Entered line kind: journal.', size=16),
        C('account_id', sa.String(26), 'Posting account selected for this entered line.', sa.ForeignKey('accounts.id'), nullable=False),
        text('side', 'Entered accounting side: debit or credit.', size=6),
        integer('amount_minor_units', 'Positive home-currency amount entered on this side.'),
        text('currency', 'Home currency of the entered amount.', size=3),
        json_text('account_snapshot', 'JSON object of account name, number, type and normal side at this revision.'),
        *dimensions(), *money_facts(),
        ...
        positive('position'), positive('amount_minor_units'), json_check('account_snapshot'), party_check(), facts_check(),
        sa.CheckConstraint("kind = 'journal' AND side IN ('debit', 'credit')", name='ck_document_line_kind_side'),
        description='Immutable ordered entered journal lines, displayed snapshots and original currency facts.')
