...
def define_tables(metadata, C, T, common):
    ...
    table('opening_evidence',ids('opening_id')+[num('ordinal','count'),*ids('transaction_id?','attachment_id?','attachment_link_id?'),obj('captured_evidence')],own('opening_id','openings','id'),fk('transaction_id','transactions','id'),fk('attachment_id','attachments','id'),fk('attachment_link_id','attachment_links','id'),
        ck('(transaction_id IS NOT NULL AND attachment_id IS NULL AND attachment_link_id IS NULL) OR (transaction_id IS NULL AND attachment_id IS NOT NULL AND attachment_link_id IS NOT NULL)'),pk='opening_id ordinal')
...
def guards(names):
    ...
    trigger('opening_evidence','attachment_owner','INSERT',"NEW.attachment_id IS NOT NULL AND NOT EXISTS (SELECT 1 FROM attachment_links l WHERE l.id=NEW.attachment_link_id AND l.attachment_id=NEW.attachment_id)")
