...
def define_tables(metadata, column, table, common):
    ...
    operation_items = T('deposit_operation_items', ident('operation_id', 'Owning receipt.'),
        text('kind', 'Complete request, deposit effect, or source component/application/allocation/document-change collection.'), integer('ordinal', 'Zero-based position in the immutable collection.'),
        ...
        check("kind IN ('request_sources','request_additional','memberships','document_changes','cash_allocations','bank_changes','source_components','source_applications','source_allocations','source_document_changes')", 'operation_item_kind'),
