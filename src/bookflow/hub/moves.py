...
def complete_company_move(s: Session, ctx: Context, row: dict[str, Any], via: str) -> dict[str, Any]:
    ...
    with s.commits.operation("moves.company", s.hub, s.company):
        pending = row["pending_path"]
        s.release_company(row["id"])
        s.commits.publishing("moves.company")
        ...
        with scope_change(s,via) as policy_effects:
        ...
        write_event(s, ctx, "company move", f"moved the folder of company {row['display_name']}",
                    [Touched("company", row["id"], "update", row["version"], new["version"], {k: v for k, v in new.items() if k in h.companies.c})] + policy_effects)
        s.commits.commit(s.hub, "moves.company")
        row.update(new)
        return row
...
def complete_org_move(s: Session, ctx: Context, org: dict[str, Any], via: str) -> dict[str, Any]:
    ...
    with s.commits.operation("moves.org", s.hub, s.company):
        ...
        for company in companies:
            s.release_company(company["id"])
        s.commits.publishing("moves.org")
        ...
        s.commits.commit(s.hub, "moves.org")
        org.update(new)
        return org
