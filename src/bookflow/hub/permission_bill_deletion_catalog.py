"""Explicit executable vendor-bill Delete delta over the accepted payment descriptor."""
from dataclasses import asdict, replace
import hashlib
import json
from . import permission_payment_deletion_catalog as previous, permission_catalog as c
from .permission_snapshot import CatalogBundle
from bookflow.core.deletion_families import BILL_FAMILIES, capability

SOURCE_COMMIT = 'cadaafb954f37e37e4b5eefdbf1f18c769e28bdf'
# The bill family has no frozen preparation to flip: its capability, its unavailable
# delete contract and its command all arrive together here, which is what makes this
# one delta the whole of what a root has to accept to gain bill deletion.
ADDED_CAPABILITIES = tuple(c.CapabilitySpec(capability(family), ('standard',), ('standard',))
    for family in BILL_FAMILIES)
...
ADDED_COMMANDS = tuple(c.CommandDescriptor(
    ...
    'bookflow.commands.bill_cmds._delete.<locals>.recover|src/bookflow/commands/bill_cmds.py', None)
    for family in BILL_FAMILIES)
ADDED_ACTIONS = tuple(c.CompanyAction(x.name,
    (c.Requirement(x.capability,'standard'),c.Requirement('ledger.read','member')),True,
    ('bookflow.commands.bill_cmds._delete.<locals>.planner|src/bookflow/commands/bill_cmds.py','bookflow.commands.bill_cmds._delete.<locals>.recover|src/bookflow/commands/bill_cmds.py')) for x in ADDED_COMMANDS)
CURRENT_SOURCES = (*previous.CATALOG.conditional_sources,
    c.ResourceSource('bookflow.company.bill_deletions.admit',
        (('src/bookflow/company/bill_deletions.py',36),),
        (c.Requirement('ledger.read','member'),)))
CATALOG = replace(previous.CATALOG, version=c.BILL_DELETE_POLICY_VERSION,
