...
Delete this credit memo with a required reason and exact expected_version. Cancel the income, the sales tax and the receivable it took back at their original dates, and release every source invoice quantity it claimed so that quantity can be returned again; retain immutable history and its number. Requires the explicit family Delete grant and ledger.read, independently of ledger.post. A credit still applied to an invoice refuses atomically with `E_HAS_APPLICATIONS` naming those invoices, and one a customer refund was paid out of with `E_HAS_REFUND` naming the refunds: take the credit back off the invoice, or void the refund, first. A closed period refuses.
...
| Capability | transaction.credit_memo.delete |
| Additional resources | ledger.read: member |
| Feature | — |
| HTTP | `POST /companies/{company_id}/commands/credit-memo.delete` |
...
|---|---|---|---|---|---|---|
| `credit_memo` | `CREDIT_MEMO` | string | yes | no | — | minimum length 1 |
...
Route: `POST /companies/{company_id}/commands/credit-memo.delete`
...
| `id` | string | yes | no | — | — |
| `family` | literal["credit_memo"] | no | no | "credit_memo" | — |
...
| `released_source_claims` | integer | yes | no | — | — |
| `source_invoice_ids` | array[string] | no | no | [] | — |
...
Page immutable credit memo revisions in revision-number order with the current header and version and their posting batches; restart on company audit changes.
...
| Feature | — |
| HTTP | `POST /companies/{company_id}/commands/credit-memo.history` |
| External binary body | none |
...
| `include_deleted` | `--include-deleted` | boolean | no | no | false | Explicitly read a retained deleted credit memo and its deletion attribution; ordinary reads omit it. |
...
Route: `POST /companies/{company_id}/commands/credit-memo.history`
...
Credit a customer. Each line debits the income account the sale used and the tax liabilities it charged, and Accounts Receivable is credited the total, so the customer owes that much less. Saving it applies nothing to any invoice: the credit stands available until it is applied or refunded. A credit memo takes the next number from the invoice series, so invoices and credits read as one unbroken run of numbers to a customer; give `number` to set one yourself, and a number already used by an invoice or another credit is refused. Each row of `lines` is either an item you name -- `item`, `quantity`, and a price exactly as an invoice line takes one -- or a return: `source_invoice` and `source_line` naming a line of a posted invoice, with `quantity` saying how much of it comes back. A returned row is priced entirely from what that invoice captured, so it takes no price, no amount and no tax code, and changing the item price or the tax rate afterwards moves none of its cents. Return the same line again and you get the next units of it; ask for more than that line has left to give back and the whole credit is refused, so nothing is written. One credit memo is all returns or all named items: the tax calculation rounds across a whole document, so a document holding both would carry a tax total that is neither. Returning a line whose item carries stock puts the goods back with the money, in this one document: the quantity goes back on hand and its cost comes out of cost of goods sold and back into the inventory asset, at what that invoice line actually took out of stock -- not at the price on the credit and not at what the item is worth today. Return half a line and half that cost comes back; return the rest later and the two halves add up to exactly what went out. Voiding the credit takes the quantity and the cost out again, and is refused, naming the date, if the goods have already been sold on. Not supported in this release, and refused rather than approximated: a price allowance against a source line without returning any of it; a line that names a stock-carrying item instead of returning the invoice line it was sold on, because a price is not a cost and nothing here guesses one -- name `source_invoice` and `source_line` to return it, or credit the money with a service or non-stock item and bring the quantity back with `inventory adjust`, which takes the value you say it is worth; a credit from one customer settling another customer or job; cash-basis treatment, since every report is accrual today; and printing, which the document print work owns.
...
| Feature | — |
| HTTP | `POST /companies/{company_id}/commands/credit-memo.post` |
| External binary body | none |
...
| `lines[].source_invoice` | inside `--lines` JSON array | string \| null | no | yes | null | — |
| `lines[].source_line` | inside `--lines` JSON array | string \| null | no | yes | null | — |
...
| `sales_tax_calculation` | `--sales-tax-calculation` | literal["line_component_half_even", "line_combined_half_up", "invoice_combined_half_up"] | no | no | null | Captured tax calculation; omission selects the company default; null rejects |
...
| `use_defaults` | `--use-defaults` | array[literal["sales_tax_calculation", "billing_address", "class_id", "customer_tax_code", "sales_tax_item"]] | no | no | [] | — |
...
Route: `POST /companies/{company_id}/commands/credit-memo.post`
...
| `type` | literal["credit_memo"] | yes | no | — | — |
| `number` | string | yes | no | — | — |
| `current_revision_id` | string | yes | no | — | — |
| `status` | literal["posted", "voided", "deleted"] | yes | no | — | — |
| `deletion` | object \| null | no | yes | null | — |
...
| `origin` | literal["standalone", "return"] | yes | no | — | — |
...
| `revision.lines[].kind` | literal["credit"] | yes | no | — | — |
...
| `revision.lines[].pricing_basis` | literal["unit", "amount"] | yes | no | — | — |
...
| `revision.lines[].source_transaction_id` | string \| null | yes | yes | — | — |
| `revision.lines[].source_revision_id` | string \| null | yes | yes | — | — |
...
| `revision.lines[].tax_components[].posting_source_id` | string \| null | yes | yes | — | — |
| `revision.lines[].tax_components[].source_tax_component_id` | string \| null | yes | yes | — | — |
...
| `revision.lines[].claims` | array[object] | no | no | [] | — |
| `revision.lines[].claims[].id` | string | yes | no | — | — |
| `revision.lines[].claims[].created_at` | string | yes | no | — | — |
| `revision.lines[].claims[].created_by` | string | yes | no | — | — |
| `revision.lines[].claims[].created_via` | string | yes | no | — | — |
| `revision.lines[].claims[].source_transaction_id` | string | yes | no | — | — |
| `revision.lines[].claims[].source_revision_id` | string | yes | no | — | — |
| `revision.lines[].claims[].source_line_id` | string | yes | no | — | — |
| `revision.lines[].claims[].start_microunits` | integer | yes | no | — | — |
| `revision.lines[].claims[].end_microunits` | integer | yes | no | — | — |
| `revision.lines[].claims[].start_quantity` | string | yes | no | — | — |
...
| `source_current.ar_account_id` | string | yes | no | — | — |
| `source_current.currency` | string | yes | no | — | — |
| `source_current.capacity_minor_units` | integer | yes | no | — | — |
| `source_current.applied_minor_units` | integer | yes | no | — | — |
| `source_current.refunded_minor_units` | integer | no | no | 0 | — |
| `source_current.available_minor_units` | integer | yes | no | — | — |
| `source_current.capacity` | object | yes | no | — | — |
| `source_current.capacity.amount` | string | yes | no | — | — |
| `source_current.capacity.currency` | string | yes | no | — | — |
| `source_current.capacity.minor_units` | integer | yes | no | — | — |
| `source_current.applied` | object | yes | no | — | — |
| `source_current.applied.amount` | string | yes | no | — | — |
| `source_current.applied.currency` | string | yes | no | — | — |
| `source_current.applied.minor_units` | integer | yes | no | — | — |
| `source_current.refunded` | object \| null | no | yes | null | — |
| `source_current.refunded.amount` | string | yes | no | — | — |
| `source_current.refunded.currency` | string | yes | no | — | — |
| `source_current.refunded.minor_units` | integer | yes | no | — | — |
| `source_current.available` | object | yes | no | — | — |
| `source_current.available.amount` | string | yes | no | — | — |
| `source_current.available.currency` | string | yes | no | — | — |
| `source_current.available.minor_units` | integer | yes | no | — | — |
...
Page credit memos in accounting-date and stable-id order, oldest first or newest first, with exact customer, receivable-account, date, number, origin and status filters. Each row carries what the credit is still worth -- its total less what has been applied to invoices and less what has been refunded -- and `available_only` keeps just the credits still worth something, which is how you find what a customer has in hand. Restart on company audit changes.
...
| Capability | ledger.read |
| Feature | — |
| HTTP | `POST /companies/{company_id}/commands/credit-memo.query` |
| External binary body | none |
...
| `limit` | `--limit` | integer | no | no | 50 | minimum 1; maximum 200 |
| `cursor` | `--cursor` | string \| null | no | yes | null | — |
| `include_deleted` | `--include-deleted` | boolean | no | no | false | Include retained deleted credit memos and their deletion attribution; ordinary pages omit them. |
...
| `ar_account` | `--ar-account` | string \| null | no | yes | null | — |
| `number` | `--number` | string \| null | no | yes | null | — |
| `origin` | `--origin` | literal["standalone", "return"] \| null | no | yes | null | — |
| `status` | `--status` | literal["posted", "voided"] \| null | no | yes | null | — |
| `available_only` | `--available-only` | boolean | no | no | false | Keep only credits that are still worth something: capacity less what has been applied to invoices and less what has been refunded. |
...
Route: `POST /companies/{company_id}/commands/credit-memo.query`
...
| `items[].type` | literal["credit_memo"] | yes | no | — | — |
| `items[].number` | string | yes | no | — | — |
...
| `items[].origin` | literal["standalone", "return"] | yes | no | — | — |
| `items[].memo` | string \| null | yes | yes | — | — |
| `items[].subtotal` | object | yes | no | — | — |
| `items[].subtotal.amount` | string | yes | no | — | — |
| `items[].subtotal.currency` | string | yes | no | — | — |
...
| `items[].total_minor_units` | integer | yes | no | — | — |
| `items[].currency` | string | yes | no | — | — |
| `items[].source_current` | object | yes | no | — | — |
| `items[].source_current.credit_source_key_id` | string | yes | no | — | — |
| `items[].source_current.party_id` | string | yes | no | — | — |
| `items[].source_current.ar_account_id` | string | yes | no | — | — |
| `items[].source_current.currency` | string | yes | no | — | — |
| `items[].source_current.capacity_minor_units` | integer | yes | no | — | — |
| `items[].source_current.applied_minor_units` | integer | yes | no | — | — |
| `items[].source_current.refunded_minor_units` | integer | no | no | 0 | — |
| `items[].source_current.available_minor_units` | integer | yes | no | — | — |
| `items[].source_current.capacity` | object | yes | no | — | — |
| `items[].source_current.capacity.amount` | string | yes | no | — | — |
| `items[].source_current.capacity.currency` | string | yes | no | — | — |
| `items[].source_current.capacity.minor_units` | integer | yes | no | — | — |
| `items[].source_current.applied` | object | yes | no | — | — |
| `items[].source_current.applied.amount` | string | yes | no | — | — |
| `items[].source_current.applied.currency` | string | yes | no | — | — |
| `items[].source_current.applied.minor_units` | integer | yes | no | — | — |
| `items[].source_current.refunded` | object \| null | no | yes | null | — |
| `items[].source_current.refunded.amount` | string | yes | no | — | — |
| `items[].source_current.refunded.currency` | string | yes | no | — | — |
| `items[].source_current.refunded.minor_units` | integer | yes | no | — | — |
| `items[].source_current.available` | object | yes | no | — | — |
| `items[].source_current.available.amount` | string | yes | no | — | — |
| `items[].source_current.available.currency` | string | yes | no | — | — |
| `items[].source_current.available.minor_units` | integer | yes | no | — | — |
| `count` | integer | yes | no | — | — |
| `has_more` | boolean | yes | no | — | — |
| `next_cursor` | string \| null | yes | yes | — | — |
| `audit_watermark` | integer | yes | no | — | — |
...
Show a credit memo: its current or a selected immutable revision, captured customer, receivable account and custom facts, its credited lines with the tax components and the source invoice quantities they claim, its posting batches, and what the credit is worth now after anything applied from it.
...
| Capability | ledger.read |
| Feature | — |
| HTTP | `POST /companies/{company_id}/commands/credit-memo.show` |
| External binary body | none |
...
Route: `POST /companies/{company_id}/commands/credit-memo.show`
...
| Capability | ledger.post |
| Feature | — |
| HTTP | `POST /companies/{company_id}/commands/credit-memo.update` |
| External binary body | none |
...
Route: `POST /companies/{company_id}/commands/credit-memo.update`
...
Void a credit memo with a required reason. Its accounting is reversed at its own date, so the income and the sales tax it took back go back where they were and the customer owes the full amount again; any stock it brought back goes out again at exactly what it came in at, leaving the quantity and the cost where the sale left them; every source invoice quantity it claimed is released and can be returned again; its number stays occupied and every revision stays readable. A credit still applied to an invoice is refused with `E_HAS_APPLICATIONS` and one a refund has paid out with `E_HAS_REFUND`: take the credit back off the invoice, or void the refund, first. A void that would take returned stock back out of an empty shelf is refused naming the date, and writes nothing.
...
| Capability | ledger.post |
| Feature | — |
| HTTP | `POST /companies/{company_id}/commands/credit-memo.void` |
| External binary body | none |
...
Route: `POST /companies/{company_id}/commands/credit-memo.void`
