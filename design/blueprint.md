...
- Folder moves are the only operation that cannot be inside a hub transaction, so the intended move is recorded before it happens. `rename --move` is: one transaction commits the new name and writes `pending_path`; the folder is moved with a no-replace rename (`renameat2` with `RENAME_NOREPLACE` on Linux, `renamex_np` with `RENAME_EXCL` on macOS, `MoveFileExW` without the replace flag on Windows), a case-only change going through `<target>.moving-<id>` recorded the same way; a second transaction sets `path` to `pending_path` and clears it. Resolution of a company whose `path` does not exist consults `pending_path` and, when that folder exists, completes the second transaction on the spot; when neither exists the error is `E_COMPANY_MISSING`. Rerunning `rename --move` with the current name completes an interrupted move and returns `moved: true`; with no pending move it returns `moved: false` and writes nothing. Organizations use the same `pending_path` on their own row, and every company path under them is derived from the organization's path plus the company folder name, so an organization move changes one row. If a registered path does not exist on open, the error is `E_COMPANY_MISSING` naming the company; there is no scan.
...
- Every directory Bookflow creates gets mode `0700` and every file `0600`, set explicitly after creation regardless of umask; `attach` refuses a folder whose mode is wider (`E_ATTACH_INVALID` with `details.check` `mode`). On Windows the default ACL of the creating user applies.
