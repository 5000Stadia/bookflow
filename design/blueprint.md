...
- **Bank feeds.** A `bank_feed_items` inbox per bank or credit card account: transactions arrive from a provider (statement import first: OFX, QFX, CSV, and parsed PDF statements through a maintained open-source statement parser chosen at that design pass; then live connections behind the same interface, SimpleFIN Bridge first for its small universal shape, then Plaid for institution coverage), are matched to existing transactions or proposed as new ones by rules the company defines (payee pattern to account, class, and memo), and are either auto-posted where a rule says so or held for approval in the inbox. Approval is the default for anything a rule does not cover; the approving user and the rule are recorded on the posted transaction.
