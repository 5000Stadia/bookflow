...
| hub_admin | bool | operator of the data root and installation-level user administration; company-book access and company administration require applicable membership, never installation status alone |
...
- **Capabilities.** Every command declares a `capability`, a dotted area such as `lists.customers`, `ledger.post`, `banking`, `payroll`, `audit`, or `settings`; by default it is derived from the command's noun. Each role maps to a default set of capabilities (table `role_capabilities`, seeded), and a membership may carry `grants` and `denies` lists of capabilities that override its role (columns on `memberships`, null by default). `design/permission-resolution.md` governs highest-role floors, scoped grant inheritance and either-scope deny precedence. Dispatch checks the role today and the resolved capability set once row 7 lands; the check has the same shape either way: `E_PERMISSION` names the capability.
...
Applications settle open items without changing document identity. Table `applications` holds immutable rows: `id`, `paying_transaction_id`, `paid_transaction_id`, exact positive home-currency `amount`, `effective_date`, `recorded_at`, `kind` (`apply`, `unapply`), nullable unique `reverses_application_id`, `audit_event_id`, and creation provenance. Payments apply to invoices, bill payments to bills, customer credits to customer receivables, and vendor credits to vendor payables. Receivable settlement lives in this table and payable settlement in `ap_applications`; both families share the same semantics -- identical exact-inverse rules, exact-dimension matching, one aging rule and one allocation function -- and a receivable row names exactly one source, a payment component key or a credit source key. A payable row names exactly one paying source too -- an `ap_source_keys` row of kind `bill_payment` or `vendor_credit` -- and settles the payable a bill owes, whichever kind it is: an open bill's balance is its gross less its effective applications with no term naming the kind, and composition is a separate read over the same rows. The payable family carries no allocation rows: `ap_applications` names the source component it consumes and the payable it answers, and nothing below the payable is attributed, because no payables reader asks which entered line was settled; when one does, it takes `application_allocations`' rule rather than a second one. A refund is not settlement: it consumes source capacity rather than settling an obligation, and has its own immutable consumption rows with the same exact-inverse rule. Both sides must belong to the same company and compatible party/AR-or-AP account; cross-party netting is an explicit future operation, never implicit. The source and target must be eligible and not voided. An application cannot predate either source document. Both documents' current versions are checked atomically.
...
Every primary list noun provides `query` with typed `summary` and `reference` projections. Input preserves the noun's text search, typed filters, active/inactive selection and ordering, and adds strict `limit` 1–200 (default 50) and an optional base64-encoded JSON cursor. The cursor is transparent: it is neither encrypted nor signed, and it carries the company id, noun, a hash of the query arguments, a hash of the caller's effective permissions, the company audit sequence, and the next offset; the server recomputes and compares every hash on each request, so a forged or altered cursor cannot widen scope, but any holder can read it. Output is `{projection, items, count, next_cursor}`; count is returned page size, not a total-result count. Reference rows have stable id, version, readable label and active state. Summary rows contain declared bounded display columns; full owned collections remain in `show`. Optional `ids` intersects the query with at most 64 stable record IDs for bounded current reference resolution; omitted IDs preserve ordinary browsing and existing fingerprints. Explicit optional `columns` selects a public per-noun catalog and returns typed descriptors, stable row identity with a `values` mapping, and a separate `matching_total`; legacy omitted-column responses retain their original shape. Strict definition-ID custom criteria support all five kinds, presence and retained inactive-definition values. `<noun> query options` pages column/filter/sort metadata and custom choices; collection columns carry counts and coordinates for bounded `<noun> query children` reads. Legacy `list` remains complete full-record enumeration and may be expensive. Filtering and ordering happen before pagination; projections do not call show once per result.
...
When a profile flag is not submitted, `service`, `non_inventory_part` and `other_charge` default to sales enabled and purchase disabled, `inventory_part` and `inventory_assembly` default both to enabled, `fixed_asset` is purchase alone, and the subtotal, group, discount, payment and sales-tax types are sales alone. A submitted flag, including `false`, is used as submitted.
...
| `/c/<company_id>/` | the home window: the flow board, rendered from the declarative map in `bookflow.adapters.workbench.home`. Panels of steps joined by arrows (Customers, Vendors, Banking, Company) where a step is a link only when its declared availability contract holds, and otherwise a dimmed, inert placeholder naming what it will do and what it waits on |
| `/c/<company_id>/_all` | every noun and verb the registry offers, in the fixed navigation groups: Company (`company`); Customers and sales (`customer`, `customer-type`, `job-type`, `sales-rep`, `price-level`, `ship-method`, `customer-message`); Vendors and purchases (`vendor`, `vendor-type`); Employees (`employee`, `other-name`); Items (`item`, `item-category`, `unit-of-measure`); Accounting (`account`, `class`, `term`, `payment-method`, `sales-tax-code`, `chart`); Settings (`custom-field`, `profile`); Audit (`audit`, `undo`) |
| `/c/<company_id>/_group/<slug>` | one entry of the persistent group menu: the same grouped-noun rendering filtered to that section |
| `/c/<company_id>/_planned` and `/c/<company_id>/_planned/<step>` | every dimmed step, what it will do, and what it is waiting on; a step whose contract has since come true redirects to its destination |
...
A tile on the home window is live only when its backing commands are registered and routed, it declares an explicit destination path, a write-labelled tile is backed by a command that writes, and this company and role offer those commands, and a test navigates to that destination and asserts a usable page. Registration and routing are discovery facts, not evidence that a handler works or that a browser page exists, so the navigation witness is part of the contract and not a courtesy check. Resolving the map reads the registry only; the home window runs no business command beyond the company lookup. Everything else is a placeholder: dimmed, `aria-disabled`, never an anchor.
