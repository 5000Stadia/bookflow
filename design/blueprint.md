...
`bookflow serve --bind 127.0.0.1:8123`. Routes are `POST /companies/{company_id}/commands/{name}` with the input model as the JSON body and the output model as the response, and `POST /commands/{name}` for hub commands, where `{name}` is the command name with spaces as dots (`company.update`), which round-trips because names never contain dots. A company may also be named by the `X-Bookflow-Company` header with the same selector rules as `--company`. Errors carry the same `{code, message, details}` document as the CLI: 401 unauthenticated; 403 `E_PERMISSION`; 404 the not-found codes; 409 `E_VERSION_CONFLICT`, `E_IDEMPOTENCY_MISMATCH`, `E_NAME_TAKEN`, `E_DB_BUSY`; 422 `E_VALIDATION`; 400 every other named error; 500 `E_INTERNAL`. The body is what matters; the status lets a client that never reads it tell not-found, conflict, and invalid input apart. `GET /openapi.json` is generated. Binding to a non-loopback address requires `--allow-network`, and the docs state that TLS termination is the deployer's job. The host also serves the workbench at `/` and static assets under `/static/`.

While the host runs it holds the data root's lock (3.2); a CLI or library call on the same machine that cannot take the lock finds the host in the lock file and sends the call, with its own context, over the host's Unix domain socket (`<data_root>/host.sock`, mode `0600`, the peer's uid taken from the kernel), so there is never a second writer and never an identity claimed over TCP. Inside the host, reads run concurrently on their own read-only connections and all writes serialize on one writer thread, so many users reading never wait on each other. Session-token liveness fields are ephemeral like presence: unversioned and unaudited.

Browser sessions: `POST /login` with username and password sets an HTTP-only, `SameSite=Lax` session cookie that maps to a session token in `api_tokens` with kind `session`, expiring after 12 hours of inactivity; cookie-authenticated command routes also require the workbench's own header, which a cross-origin form cannot send. API calls from programs send a bearer token. Both resolve to the same context. Every login failure is one code, `E_LOGIN_FAILED`.
...
| `/c/<company_id>/<noun>/<verb>` | a form generated from the input model: one input per field, typed, with the field description, a clear box per nullable field, and `reason` and `directive` fields on writes; an update form carries `expected_version` from the show it rendered; submit runs the command and renders the output model and any error with its code |
...
| Full test suite under 120 s serially | measured 70 s at 169 tests; subprocess CLI tests dominate |
