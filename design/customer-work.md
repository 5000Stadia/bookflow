...
The non-posting portion of [Customer work and billing](customer-work-and-billing.md)
and [customer-work controls](inventories/customer-work.md) has shared commands and browser forms. Financial conversion,
partial/progress allocations and customer payments remain subsequent increments;
...
`customer-work` resource and the hub seeds its role defaults. Granular capability
overrides and agent-principal authority remain the Row7 target.
...
version, duplicate-number and stale-query/preview errors. E_WORK_DEPENDENCY covers
...
Company migration co0010 uses revision-local DDL and preserves every old row. The sole old-table
...
The work tables are work_documents (primary identity/kind/number/version/current revision/status/
...
Annotation targets include work_document, work_revision and work_line; exact source
...
Dedicated work_models/facts/schema/service/validation modules and thin
work_cmds registry adapters implement the contract. Shared-commercial factoring is restricted to reusable
...
and generated docs/inventory/error matrix. Preserving upgrades retain local scope columns, generated values and constraints
while widening the known scope CHECK; unrecognized definitions fail atomically.
Reopening complete work clears its old end before validating a new start, and
