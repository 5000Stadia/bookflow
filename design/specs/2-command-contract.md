...
`UpdateOutput{dry_run, warnings, idempotent_replay, version, changed_fields, merged_over_versions (empty list on a blind write), previous_* (null on a versioned write), recent_concurrent_activity}`. The version a versioned write names is the record's own: for `company update` it is `info.version` as shown by `company show`, never the registration row's `version`, and the help text of `--expected-version` says so.
...
`company update` takes every `company_info` column except `id`, the common fields, `display_name`, `home_currency`, and `default_chart`, as top-level fields with the three addresses nested; a nested address given partially patches the children given and leaves the rest. It requires the admin or owner role per blueprint 4.3, `closing_date` included. Output adds `company_id`; the event summary names the fields changed. When `legal_name` changes, the hub projection is updated in the same command: `writes` is `{company, hub}`, the company event carries the `company_info` entry and the hub event the registration entry, joined by `request_id`.
...
Tables `idempotency_keys` in hub.db (hub0002) and company.db: `actor_id`, `key`, `command`, `input_hash` (sha256 of the canonical JSON of the validated input plus the company id), `request_id`, `output` (JSON), `created_at`. The output is stored, not a result id, because a replay must return what the caller would have seen without re-running anything. Keys are accepted on `create` and `post` commands and on `company new` and `organization new`, per blueprint 6.5. In dispatch, when the context carries a key: before `plan`, look it up in the database the command writes; a hit with the same command and `input_hash` returns the stored output with `idempotent_replay: true` and writes nothing; a hit with a different command or input is `E_IDEMPOTENCY_MISMATCH`; a hit older than 30 days is ignored at lookup and replaced by the store. After `apply`, the output is stored in the same transaction, or in a final transaction dispatch opens for commands that commit their own. A dry run with a used key returns the stored output with `dry_run` and `idempotent_replay` true. Keys older than 30 days are deleted by the next store. `idempotent_replay` lives on `WriteOutput`.
...
`Context.directive_id` carries the selector as given by the adapter; dispatch resolves it and copies the id in. `--directive` exists only on company-scope writes, since directives live in the company. After actor resolution, when the actor kind is `agent` or `system` and the command writes anything, `reason` or `directive_id` is required, else `E_REASON_REQUIRED`; humans are never required to give either. Blueprint 5.8 is amended from ledger-posting commands to every write, so that an agent's writes always record why, which is what the intention's third quality says. Agents have no surface until row 7; the fixture user of kind `agent` stands in, `on_behalf_of` is null until tokens exist, and both facts are recorded in `architecture.md`.
...
Only `company new`, `company rename`, and `company update` with `legal_name` write both databases. Order: company transaction with its event and idempotency store, commit; then hub transaction with its event, commit. A failure after the company commit returns `E_INTERNAL` with `details.request_id`; the company write is durable and audited, the hub side is not. Retrying with the same key replays the stored company output; a retry without a key reruns and, for `legal_name`, converges the projection. Tests inject a failure at each cross-database commit boundary and assert what remains durable.

## Event ordering

`audit_events` in both databases gains `seq`, an integer that SQLite assigns in insertion order (`INTEGER PRIMARY KEY AUTOINCREMENT` on a companion rowid, exposed as `seq`), written under the data-root lock. Cursors for `list` and `tail` are `seq` values, so same-millisecond events and process restarts never reorder the feed; the event `id` stays a ULID for citation.
...
- Does not touch: lists, ledger, attachments, tokens, HTTP. `undo` (blueprint 7) is assigned to row 5, the first row with list records to undo.
