...
src/bookflow/
  ...
  hub/
    ...
    moves.py             complete_company_move() / complete_org_move(): finish a pending move from any state with a version bump and a move event; effective_path() for read-only opens
  company/
...
- A writable hub open completes any pending company or organization move before the command plans (hub/moves.py); read-only opens use whichever folder exists and write nothing.
...
- Cold start: `bookflow --help` about 280 ms against the 300 ms budget (`BOOKFLOW_BUDGET_MS` overrides the test on slower machines); SQLAlchemy is not imported for help.
...
- Agents cannot yet exist through any surface; the CLI maps an OS login to whatever user `config.toml` names (tests use that to stand in for other users until row 7).
