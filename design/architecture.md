...
The private ordinary DepositOperationView now validates original submitted post/update/void input, exact supplied-field presence, and agreement between stored request, operation receipt, document, audit event, financial identity map and optional consumed draft. Draft pins and resolution maps remain private validation evidence; complete captured business composition is carried by the receipt. Whole-event decoder registration and masking are still pending, including typed traversal of draft custom-value maps and the coordinate family.
