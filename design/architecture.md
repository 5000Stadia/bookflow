...
Private deposit audit capture preparation adds closed ordinary lifecycle and consumed-composition views, validating original receipt/manifest contracts before projection. Consumed snapshot JSON becomes a typed composition; omitted legacy draft fields stay omitted. Internal fingerprints are not display fields. Private capture decoders do not activate public audit history. Draft/selection touched-record integration, coordinate receipt routes and complete producer/publication coverage remain required.

The private ordinary DepositOperationView now validates original submitted post/update/void input, exact supplied-field presence, and agreement between stored request, operation receipt, document, audit event, financial identity map and optional consumed draft. Draft pins and resolution maps remain private validation evidence; complete captured business composition is carried by the receipt. The private company event decoder admits ordinary deposit post/update/void operation captures only for matching create entries and commands. Its existing event admission and field projection apply before publication. Draft/selection touched-record integration remains pending; the private coordinate operation decoder uses its own closed receipt owner.

Private deposit capture disclosure uses explicit reference maps and whole-field capability checks for custom maps. Denied polymorphic parties remove both the reference and captured label; absent and denied party origins normalize identically. Reference-dependent origin entries and internal supplied-field assertions are omitted under their current owners. Nullable projection annotations retain captured-nonnull validation. Real bound-reader/current-policy tests cover exact retained totals and no company mutation, but do not substitute for whole-event admission, full producer coverage or transport publication.

Private coordinate audit source collections reuse twenty explicit existing row views, omitting event-level tags inside the captured receipt. The source collection and header validate the original coordinate owner before decoding; typed snapshot conversion, private-field omission and inherited disclosure routes remain those of the existing row views. Coordinate source views inherit the existing row disclosure rules. The private event decoder selects the coordinate operation owner explicitly; cross-record authority and public event integration remain separate requirements.

Private coordinate audit receipts decode all four source-action forms, bank movements, custom changes, original operation intent and item manifests. Original owner schemas validate raw input before projection; the typed receipt additionally binds target/current headers, prior source version, before-plus-inserted row composition, shared aggregate identities and optional draft consumption. Item manifests are recomputed through the existing pure operation collection owner. Raw bank provenance and internal request guards remain validation evidence, excluded from display. The private disclosure walker consults coordinate reference and field requirements explicitly, including original-input field presence, without mutating legacy descriptor maps. The event decoder accepts coordinate operations only for matching deposit-coordinate create captures. Full-event permission acceptance and public publication remain separate requirements. Operation item rows are paged storage, not separate audit entries; their manifests are validated inside the captured operation receipt.
