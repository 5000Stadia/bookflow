...
`deposit-deletion-v1`, `job-time-v1` and then `journal-deletion-v1`, which is the current
catalog. Journal entry is the last of the four families the frozen ancestor prepared: its
capability and its unavailable delete contract were declared there long ago, so its delta
flips them rather than adding them.
