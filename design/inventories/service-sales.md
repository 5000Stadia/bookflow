...
| W2 | Proposal/scope, estimate, order and work-order conversion; full, partial/progress or selected-line billing; remaining work | [Customer work and billing](../customer-work-and-billing.md) supports linked proposals/estimates/work orders, whole and partial work billing, previous/current/cumulative/remaining measures and exact rebilling. Estimates-versus-actuals reports, formal change-order approval and stock-order fulfillment remain staged. Completing work does not itself prove payment. |
...
| W10 | Full/partial payments; automatic total calculation; automatic or manual allocation; unapply/reapply | Staged: independent automatic-calculation and automatic-application preferences. Automatic calculation supports a blank payment amount derived from selected invoices, and calculated per-invoice allocations when received cash is supplied. Explicit cash entry is distinguished from a selection-derived draft total. Automatic application offers matching amounts/oldest-invoice suggestions; per-invoice amounts remain editable and over/underpayment differences visible. Allocations require explicit application history, eligible party/account and amount limits. |
