...
| 16 | Non-posting customer work documents. Proposal/statement of work, alternative estimates, explicit accepted revision, and work orders with scheduling and completed quantities; shared captured commercial facts, exact cost/markup/amount pricing, permanent conversion lineage, notes/files and typed custom fields. See [owning plan](specs/16-customer-work.md). | All 21 commands work through the shared library/CLI/HTTP contract and readable desktop/phone forms. A proposal becomes alternatives, the accepted estimate becomes one work order, and completion preserves history without creating any ledger effect. Retried conversion, stale edits, alternative selection and immutable line ownership are enforced; both existing demo histories and balances survive the additive extension. |
| 17 | Linked work billing. Accepted estimates and work orders become invoices or paid sales receipts with exact captured whole-line facts, shared billing-root consumption, permanent retry identity, amount-priced sales and source-linked correction/void. See [owning plan](specs/17-work-billing.md). Partial quantity/amount/percentage billing follows in the next increment. | Six shared commands and readable desktop/phone forms bill selected whole lines or their unbilled remainder; an ancestor and its work order cannot bill the same root twice. Invoice/receipt corrections and voids preserve exact accounting and release only their own work. Amount-priced quotes retain their exact net without a fictitious rate; existing histories, files and demo balances survive. |
