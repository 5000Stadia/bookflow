...
|---|---|---|
| 10 | Storage guarantees. Read commands observe internally consistent hub and company snapshots. Authoritative SQLite commits use durable synchronization; metadata replacement and directory moves preserve recoverable durable ordering. Streaming drains bounded batches and releases snapshots between batches. | A concurrent customer edit cannot produce a mixed-version show or list response; dry runs remain write-free; error paths release readers; blocked checkpoints do not weaken commit durability; filesystem failure/retry and migration witnesses pass. |
| 11 | Bounded company queries. Every primary list noun exposes typed summary and reference pages through the shared command registry, with bounded database work and explicit continuation. Existing list enumeration and full show records remain compatible. | All 20 nouns support bounded query pages; unchanged data enumerates exactly once; cursor mismatch and concurrent changes are explicit; filtering/sorting occurs before pagination; query count is independent of result count; 10,000-record interactive witnesses meet the declared budget. |
| 12 | Customer/job browser workflow and shared reference controls. Bounded searchable lists lead to readable customer/job details and grouped editing; create-job preserves parent context without changing inheritance semantics. Human-readable reference selection preserves stable identifiers and version checks underneath. Generated forms remain available for every command. | A human can find a customer, inspect its jobs and contact/inheritance context, create a job, edit and preview/save a record, select and create related records by label, and understand a stale-edit error without losing input. Nested item/unit/vendor references work in an actual browser; keyboard and narrow-screen tests pass; the human receives a concrete functionality-testing checklist. |
...
| 8 | General ledger. Transactions as header plus lines, journal entry command, posting rules, void, closing date, and the trial balance and general ledger reports. Money type with home currency, exchange rate table, and foreign-tagged entry converting at posting. The browser UI gains register-style entry: type a row, tab across its fields, and post without working directly with debit and credit lines. | A balanced journal entry posts from the register, the generated workbench, and the CLI; an unbalanced one is rejected, a void reverses it, posting before the closing date is rejected, and the trial balance balances. Register entry supports keyboard-only row entry and makes the posted result visible in place. A JPY-tagged line posts in home currency and keeps the original amount and rate. |
