...
| 14 | Standard accrual financial statements over the existing immutable ledger: profit and loss for an inclusive date range, and balance sheet as of a date. Exact normal-side account amounts, complete statement totals, fiscal-year earnings, bounded continuation, readable browser output and account-ledger drill-down. | Both commands work through Python, CLI, HTTP and browser; the reference year reconciles to trial balance and source arithmetic, including corrections/voids, contra balances and fiscal rollover. Report reads write nothing; cash basis is explicitly unsupported. |
