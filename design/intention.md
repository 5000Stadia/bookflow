...
|---|---|---|
| 15 | Service invoices and sales receipts. Home-currency commercial item lines, customer/job defaults, exact prices/units/terms/tax, immutable profiles and effects, correcting edits and voids through the shared command registry. Detailed supported forms and staged dependencies in design/specs/15-service-sales.md. | A customer service invoice and paid sales receipt can be entered, previewed, posted, corrected and inspected on desktop/phone and through Python/CLI/HTTP. Commercial totals, captured facts, balanced effects and source attribution reconcile; history, custom fields, isolation, idempotency and closed periods hold. Existing journals and copied company files survive migration. |
