...
|---|---|---|
| 13 | Local performance tracing. Human: "observe the process, file, database mechanism latencies involved in certain functions". Add opt-in finite local capture of command stages, queue/lock waits, database execute/fetch/commit and explicit file synchronization, with private bounded export and no sensitive contents or telemetry service. | A diagnostic run distinguishes application startup from hosted work and queue waiting from execution; traces cover raw database commits and file sync without changing command results, audit, durability or cleanup. Concurrent operations remain correctly attributed, capture limits and failures are safe, exported timelines are readable, and traced/untraced overhead is measured separately from strict uninstrumented budgets. |
